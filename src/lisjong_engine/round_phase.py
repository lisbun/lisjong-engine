...
class RoundPhase(Enum):
    ...
    `AWAITING_RIICHI_DISCARD`は、current seatが立直を選択した一方、
    宣言牌をまだ選んでいない中間状態を表す。この段階では手牌・河・
    drawn tileを変更せず、current seatだけが立直宣言可能な打牌を選ぶ。
    立直の宣言と宣言牌の選択が別のselector decisionであることを、独立
    したphaseにすることで表現する。
